...
- **Arte visual explicativa não é trabalho para SVG improvisado**: não voltar a tentar “arte”, capa, diagrama bonito ou infográfico sedutor em SVG manual quando o objetivo for apelo visual real. SVG manual serve para esquema técnico seco; para artefato visual de livro, preferir imagem gerada, ilustração raster ou designer/codepath realmente visual.
...
- **Caminho do container não é caminho do host**: se uma app no Coolify enxergar algo como `/data/...` dentro do container, não assumir que o host usa o mesmo path. Primeiro inspecionar o mount real (`docker volume inspect` ou bind mount) e só então copiar arquivo ou validar presença; `docker exec` é a prova final do que o container realmente vê.
...
- **Tecnologia pede subcategoria real**: ao cadastrar livro em `Tecnologia`, não parar na categoria pai por preguiça. Escolher a subcategoria correta (`IA`, `Backend`, `Frontend`, `Dados`, `Cloud`, `DevOps` etc.). No script atual, `--category-name` resolve só categoria raiz; para subcategoria, usar `--category-id`.
...
- `create-book.md`: fluxo oficial para transformar manuscrito em livro com capa, HTML/CSS e PDF final limpo.
