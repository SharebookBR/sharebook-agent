...
- Qualquer operação na fila de importação de ebooks: triagem, publish, worker, `retry_later`, `error`, `source_blocked`, ciclo manual Windows, scripts → abrir `sharebook-agent/skills/importers/ebook-importer/SKILL.md`.
...
- SEO, GA4, GSC, funil, tráfego, landing pages ou auditoria de indexação → abrir `sharebook-agent/skills/engineering/analytics/SKILL.md`.
...
- Gestão de categorias, taxonomia, migração de leaf category ou revisão de hierarquia → abrir `sharebook-agent/skills/importers/category-organizer/SKILL.md`.
