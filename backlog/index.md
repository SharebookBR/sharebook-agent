...
2. **[Simplificação e modernização do código (backend)](todo/simplificacao-modernizacao-backend/index.md)** — alto valor, esforço ainda não estimado. Continuação natural da mesma frente do frontend ([concluída em 2026-09-19](done/simplificacao-modernizacao-frontend/index.md), validada em dev pelo Raffa), agora no `sharebook-backend`: mesmo objetivo de reduzir custo cognitivo e tornar o código humano/IA-friendly, sem apego a arquitetura histórica nem a patterns modernos por si só. Pedido do Raffa em 2026-09-19. Diagnóstico entregue e revisado; tarefas 2-4 concluídas, execução em andamento.
3. **[Débitos técnicos backend](todo/debitos-tecnicos-backend.md)** — valor médio, esforço baixo. Dois achados incidentais durante o épico acima: teste `HelperTests.ImageResize` que faz chamada HTTP de verdade pra URL externa de terceiro (frágil por design, não só na sandbox), e migration `RenameEFLogs` com nome de índice hardcoded do banco de produção que impede rodar a cadeia de migrations do zero num ambiente limpo.
