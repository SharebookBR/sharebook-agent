...
2. **[Simplificação e modernização do código (frontend)](todo/simplificacao-modernizacao-frontend/index.md)** — alto valor, esforço alto fatiável. `sharebook-frontend` já está em Angular 22 ([Dependências e Segurança](done/seguranca-e-vulnerabilidades.md) — **concluído em 2026-09-18**, vulnerabilidades de produção 105→4), mas a migração não tocou estrutura, organização nem padrões, só o motor. Prioridade #1 do épico, por pedido do Raffa: reorganização de pastas por domínio (custo cognitivo e descoberta), com desapego total ao passado. Demais tarefas (hydration SSR, subscribes sem catchError, interceptors funcionais, rede de testes, lazy loading/standalone) fatiadas em `todo/simplificacao-modernizacao-frontend/tarefaNN-*.md`.
3. **[Simplificação e modernização do código (backend)](todo/simplificacao-modernizacao-backend/index.md)** — alto valor, esforço ainda não estimado. Continuação natural da mesma frente do frontend, agora no `sharebook-backend`: mesmo objetivo de reduzir custo cognitivo e tornar o código humano/IA-friendly, sem apego a arquitetura histórica nem a patterns modernos por si só. Pedido do Raffa em 2026-09-19. Ainda na fase de diagnóstico — a Tarefa 1 (`todo/simplificacao-modernizacao-backend/tarefa01-diagnostico.md`) precisa ser entregue e revisada antes de qualquer refatoração começar.
