...
3. **[Dependências e Segurança](todo/seguranca-e-vulnerabilidades.md)** — alto valor, alto esforço. Triar o que é realmente explorável e modernizar incrementalmente, sem salto cego de major. Angular 13 está end-of-life; plano de migração hop a hop até LTS em [`migracao-angular-13-lts.md`](todo/migracao-angular-13-lts.md). **Atualizado 2026-09-17**: hops 13→16 concluídos e pushados (`sharebook-frontend`, branch `claude/angular-lts-migration`); vulnerabilidades de produção caíram de 105 para 22 (só com o salto de versão + `npm audit fix`, sem breaking change). Resta continuar até LTS (20) e migrar `@nguniversal`→`@angular/ssr`.
