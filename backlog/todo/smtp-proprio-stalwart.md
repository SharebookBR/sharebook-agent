...
- Em 2026-09-06, `mail.sharebook.com.br` passou a resolver para `<VPS_HOSTGATOR_SSH_HOST do .env>` e o PTR de `<VPS_HOSTGATOR_SSH_HOST do .env>` passou a apontar para `mail.sharebook.com.br`.
...
- Em 2026-09-08, DNS de autenticação foi publicado e validado publicamente: SPF raiz, DMARC `p=none`, MX/SPF de `bounces.sharebook.com.br` e quatro DKIM (`v1-rsa-20260906` / `v1-ed25519-20260906` para domínio raiz e bounce). A resolução direta/reversa também bate (`mail.sharebook.com.br` ↔ `<VPS_HOSTGATOR_SSH_HOST do .env>`).
...
- [x] Garantir que bounces assíncronos continuem alimentando `MailBounces` e a lista de supressão. (2026-09-08: DSN real processado por `Operations/JobTest`; `MailBounces` recebeu `destinatario-inexistente@example.com`, erro `550`, hard bounce)
