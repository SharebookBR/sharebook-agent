...
- **Falso verde do `trust`.** O primeiro teste da senha rotacionada passou com a senha certa **e** com a vazada. Foi a errada passando que denunciou. Refeito pelo IP do container, que cai em `scram-sha-256`.
  - **Auditado no fim da sessão, a pedido do Raffa:** o `trust` vale só para socket local, `127.0.0.1/32` e `::1/128`; o resto cai em `scram-sha-256`. Container em rede bridge (`coolify`), então o loopback dele é namespace próprio — testei do `sharebook-api` e não alcança. Porta não publicada no host, grupo `docker` vazio, só `root` com shell. Explorar exige já ser root, e root não precisa de senha de Postgres. **Não é vulnerabilidade, é o padrão da imagem oficial** — e trocar a linha para `scram` quebraria o `pg_dump` sem senha do backup do Coolify.
...
- **Descrevi um achado de um jeito que assustou.** Escrevi "a senha vazada funcionou" e "foi a senha errada funcionando que denunciou o furo". O Raffa leu aquilo como brecha de autenticação e voltou preocupado, já com a sessão fechada. A frase correta era **"o teste não verificava senha nenhuma"** — o defeito estava no meu teste, não no banco. Num relatório de segurança, sujeito errado na frase vira incidente imaginário. Corrigido na skill (`7273a32`) para não assustar o próximo leitor.
