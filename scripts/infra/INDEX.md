...
- `sharebook_agent_send_email.py` — envia e-mail pelo SMTP próprio do Sharebook/Stalwart usando `SHAREBOOK_AGENT_SMTP_*` do `.env`; no OpenClaw usa túnel SSH pela VPS (`SHAREBOOK_AGENT_SMTP_VIA_VPS=true`) para alcançar o Stalwart interno. Suporta texto/HTML, `--body`, `--body-file`, CC/BCC, Reply-To e `--dry-run`.
