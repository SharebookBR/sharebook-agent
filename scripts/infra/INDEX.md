...
- `sharebook_agent_send_email.py` — envia e-mail pelo SMTP próprio do Sharebook/Stalwart usando `SHAREBOOK_AGENT_SMTP_*` do `.env`; no OpenClaw usa túnel SSH pela VPS (`SHAREBOOK_AGENT_SMTP_VIA_VPS=true`) para alcançar o Stalwart interno. Suporta texto/HTML, `--body`, `--body-file`, CC/BCC, Reply-To, `--envelope-from`/Return-Path e `--dry-run`.
- `sharebook_bounce_imap.py` — inventaria e processa bounces assíncronos recebidos por IMAP no Stalwart usando `SHAREBOOK_BOUNCE_IMAP_*` do `.env`; por padrão faz dry-run read-only, e só com `--process` grava `MailBounces` e expunge apenas os UIDs parseados como bounce.
