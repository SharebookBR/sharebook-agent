...
O sink do Rollbar tem filtro próprio em `ShareBook.Api/Logging/RollbarLogEventFilter.cs`. Eventos `Microsoft.EntityFrameworkCore.Database.Command` são a cópia de baixo nível de uma exceção registrada depois com a causa real e não devem abrir um segundo item. A violação conhecida de unicidade de `Users.Email` também é excluída quando aparece em `Microsoft.EntityFrameworkCore.Update`, porque o cadastro a converte em `409 Conflict`. O filtro é exclusivo do Rollbar: console e demais logs continuam recebendo os eventos. Não baixar globalmente o nível de `Microsoft.EntityFrameworkCore.Update`, pois isso esconderia falhas reais de persistência.
