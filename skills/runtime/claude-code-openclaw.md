...
- **Existe mais de um token GitHub no `.env`** (ex.: `GITHUB_TOKEN_SHAREBOOK_FRONTEND`, específico do repo, e `GITHUB_PERSONAL_ACCESS_TOKEN`, genérico). Um deles pode expirar sem aviso — `git push` falha com "could not read Username", o mesmo sintoma de um `extraheader` mal formado. Antes de trocar de token achando que o formato está errado, testar a validade direto contra a API (`GET /repos/<org>/<repo>` com o header `Authorization: Bearer <token>`, checar status code) — só isso distingue token morto de configuração errada. Se expirado, seguir com o token genérico que ainda funciona e avisar o Raffa pra rotacionar o específico.
- **`git checkout <outro-ref> -- .` estando numa branch diferente é perigoso mesmo sem `--force`**: estagia o conteúdo inteiro do outro ref no índice da branch atual, sem avisar que está fora de contexto — fácil de gerar um commit de doutrina corrompido na branch errada sem perceber. Pra inspecionar ou trabalhar em cima de outro ref, sempre criar uma branch local dedicada primeiro (`git checkout -B <nome> origin/<nome>`), nunca misturar refs no índice da branch corrente.
