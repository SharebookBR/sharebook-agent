...
- O estado de modelos validado nessa ativação usa `openai/gpt-5.5` como primário e `deepseek/deepseek-v4-pro` como fallback global. O provider `deepseek` usa `DEEPSEEK_API_KEY`; não registrar a chave no repositório. Fallbacks globais são acionados para falhas elegíveis do provider (por exemplo, autenticação, rate limit e timeout); uma seleção explícita de modelo na sessão permanece estrita.
...
- `gateway.controlUi.dangerouslyDisableDeviceAuth` deve permanecer ausente. Notebook, celular e qualquer perfil novo de navegador usam identidade própria e pareamento; limpar os dados do site ou usar janela privada cria outro dispositivo.
- O wrapper aplica `AUTH_PASSWORD` via `auth_basic` e também injeta o gateway token nas requisições encaminhadas. Em 2026-08-30, após prompts repetidos causados inclusive por `/__openclaw__/assistant-media`, Raffa decidiu remover deliberadamente o Basic Auth e assumir a exposição adicional. **Isto não equivale a uma borda autenticada segura**: o domínio continua público e rotas HTTP recebem o token injetado pelo wrapper. Device pairing é a barreira da Control UI, não uma proteção geral para toda superfície HTTP. Se a postura mudar, preferir Cloudflare Access ou Tailscale antes de reabrir o domínio.
- O deployment instala o hook [`scripts/infra/openclaw_disable_basic_auth_init.sh`](../../scripts/infra/openclaw_disable_basic_auth_init.sh) no volume persistente em `/data/openclaw-init/disable-nginx-basic-auth.sh` e define `OPENCLAW_DOCKER_INIT_SCRIPT` com esse path. O hook aguarda o nginx gerado pelo wrapper, remove `auth_basic` e `auth_basic_user_file` de todas as localizações, valida com `nginx -t` e recarrega o nginx. É idempotente. Preservar a variável na configuração do serviço, pois editar apenas o nginx vivo não sobrevive a restart.
- Neste deployment, `gateway.trustedProxies` contém somente `127.0.0.1` e `::1`, fazendo o Gateway reconhecer o nginx local sem transformar toda a rede Docker em proxy confiável.
