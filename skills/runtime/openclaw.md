...
- O wrapper aplica `AUTH_PASSWORD` via `auth_basic` em todas as rotas, exceto `/healthz` (e `/hooks` quando hooks estão habilitados). Na Control UI, requisições estáticas como `/sw.js`, `/manifest.webmanifest`, `/control-ui-config.json` e `/avatar/*` podem omitir Basic Auth e gerar prompts `401` repetidos no navegador, mesmo com a página e o WebSocket funcionando; isso foi observado no deployment Sharebook em 2026-08-30. Não remover `AUTH_PASSWORD` como atalho: sem uma borda autenticada adicional, o nginx injeta o gateway token e o serviço fica exposto. Soluções duráveis são corrigir/atualizar o wrapper para isentar apenas assets públicos comprovados, ou colocar uma autenticação de borda compatível com SPA/WebSocket e reativar a autenticação de dispositivo.
