...
- O wrapper aplica `AUTH_PASSWORD` via `auth_basic` em todas as rotas, exceto `/healthz` (e `/hooks` quando hooks estão habilitados). Na Control UI, requisições estáticas como `/sw.js`, `/manifest.webmanifest`, `/control-ui-config.json` e `/avatar/*` podem omitir Basic Auth e gerar prompts `401` repetidos no navegador, mesmo com a página e o WebSocket funcionando; isso foi observado no deployment Sharebook em 2026-08-30. Não remover `AUTH_PASSWORD` como atalho: sem uma borda autenticada adicional, o nginx injeta o gateway token e o serviço fica exposto.
- O deployment Sharebook instala o hook [`scripts/infra/openclaw_static_auth_init.sh`](../../scripts/infra/openclaw_static_auth_init.sh) no volume persistente em `/data/openclaw-init/patch-nginx-static-auth.sh` e define `OPENCLAW_DOCKER_INIT_SCRIPT` no serviço Coolify. O hook aguarda o nginx gerado pelo wrapper e insere uma única localização regex, sem Basic Auth, somente para os assets públicos comprovados (`sw.js`, manifest/config da UI, ícones, `assets/`, `avatar/` e `provider-icons/`); `/` e `/browser/` continuam protegidos. É idempotente e valida/recarrega o nginx; a variável precisa ser preservada se o serviço for reconfigurado e o Coolify regenerar o `.env`.
