...
- **Cartão > Tabela**: Para listas operacionais (como o importador), prefira cartões compactos e responsivos. Tabelas são hostis em mobile.
- **Smart Sorting**: Automatize a ordenação baseada no status (ex: fila de espera -> posição ASC; concluídos -> data DESC).
- **Inspetor de Metadados**: Nunca exiba JSON bruto. Use flattening recursivo e listas zebradas para inspeção humana.
- **Feedback de Sucesso**: Em fluxos de publicação, a miniatura do ativo criado (ex: capa do livro) no card de conclusão é o melhor feedback visual.
